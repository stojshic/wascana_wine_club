			...
			<div class="bg-white rounded-xl shadow p-4">
				<div class="flex gap-3 mb-3">
					...
					<div class="flex-1 min-w-0">
						<p class="font-semibold text-gray-900">{wine.name}</p>
						<p class="text-sm text-gray-500 truncate">{wine.description}</p>
					</div>
				</div>
				<div class="flex items-center justify-between pt-3 border-t border-gray-100">
					<div class="flex items-center gap-4">
						<span class="font-semibold text-gray-900">${wine.price.toFixed(2)}</span>
						{#if wine.stock === 0}
							<span class="text-red-600 text-sm font-medium">Out of stock</span>
						{:else if wine.stock <= 5}
							<span class="text-amber-600 text-sm font-medium">{wine.stock} left</span>
						{:else}
							<span class="text-green-600 text-sm font-medium">{wine.stock} in stock</span>
						{/if}
					</div>
